...
export const Button = ({ variant = "primary", size = "md", ...props }: ButtonProps) => {
  ...
  return (
    <AnimatedPressable onPressIn={(e) => { }} onPressOut={(e) => { }} className={cn(Variants[variant].container, props.className)} {...props}>
      <Text className={cn(`text-${size}`, Variants[variant].text)}>{props.children}</Text>
    </AnimatedPressable>
  );
};
